...
public class TopNewsService {
    ...
    private final String TOPNEWS = "https://newsapi.org/v2/top-headlines";

    @Value("${API_KEY}")
    private String apiKey;

    public List<TopNews> getNewsList(String country) { 
        //removed String query
        //String apiKey = System.getenv("API_KEY");

        String url = UriComponentsBuilder.fromUriString(TOPNEWS)
        .queryParam("country", "country=sg")
        .toUriString();

        RestTemplate template = new RestTemplate();
        ResponseEntity<String> resp = null;
        List<TopNews> list = new LinkedList<>();
